...
The final go/no-go audit is `python .\scripts\hn_readiness.py --health-url <url> --dns-fixture <launch-kit\dns-fixture.json> --launch-kit <launch-kit> --file-or-hash <file-or-hash> --domain <domain> --status-base-url <url> --doh-endpoint <url> --evidence-out <path>`, which verifies launch-kit artifact checksums, posts the same public file or hash to the deployed `/api/verify` endpoint, requires PASS, and writes machine-readable evidence.
