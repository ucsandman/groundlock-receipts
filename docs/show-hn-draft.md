...
The final go/no-go audit is `python .\scripts\hn_readiness.py --health-url <url> --dns-fixture <dns-fixture.json> --file-or-hash <file-or-hash> --domain <domain> --status-base-url <url> --doh-endpoint <url> --evidence-out <path>`, which also posts the same public file or hash to the deployed `/api/verify` endpoint, requires PASS, and writes machine-readable evidence.
...
- `scripts/hn_readiness.py --evidence-out <path>` passing and writing JSON evidence with `ok: true`
