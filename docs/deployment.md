...
- `--launch-kit` is provided and the launch kit is missing, has mismatched summary fields, has summary receipt metadata, TXT count, or status count that does not match the DNS fixture, has a missing or invalid summary fetch timeout, has `dns-zone.txt` TXT records that do not match the DNS fixture, has `web.env` values that do not match the readiness inputs, has an invalid `web.env` fetch timeout, has a `web.env` fetch timeout that differs from `launch-summary.json`, has bundled status records that do not match the DNS fixture, has `status-records.json` entries that do not match the DNS fixture status records, has artifact hashes that do not match the files, has a checksum manifest that does not match `launch-summary.json`, has a copied fixture that differs from `--dns-fixture`, or contains private-key markers in public handoff artifacts
