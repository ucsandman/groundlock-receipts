...
- deployed `robots.txt` does not point to the public sitemap and disallow API/status routes, or deployed `sitemap.xml` omits the public homepage or threat model URLs, includes stale placeholder/local origins, exposes API/status paths, or is invalid XML
