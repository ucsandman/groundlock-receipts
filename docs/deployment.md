...
`GROUNDLOCK_RATE_LIMIT_MAX` and `GROUNDLOCK_RATE_LIMIT_WINDOW_MS` are optional positive integers. Defaults are `240` verifier requests per `60000` ms per app instance. Invalid configured values return `/api/health` `503` with `invalid_rate_limit` in live mode and make `/api/verify` fail closed with `rate_limit_invalid`. The public verifier intentionally does not trust spoofable forwarding headers for client identity, so put stricter per-client throttling at a trusted edge proxy if needed.
...
- `launch-summary.json` - the launch domain, site URL, content hash, receipt hash, signer key id, fetch timeout, artifact names, and SHA-256 hashes for the public handoff artifacts.
...
- `--launch-kit` is provided and the launch kit is missing, has mismatched summary fields, has summary receipt metadata, TXT count, or status count that does not match the DNS fixture, has a missing or invalid summary fetch timeout, has `dns-zone.txt` TXT records that do not match the DNS fixture, has `web.env` values that do not match the readiness inputs, has an invalid `web.env` fetch timeout, has a `web.env` fetch timeout that differs from `launch-summary.json`, has bundled status records that do not match the DNS fixture, has `status-records.json` entries that do not match the DNS fixture status records, has `hn-readiness.ps1` or `runbook.md` arguments that do not match the readiness inputs, has artifact hashes that do not match the files, has a checksum manifest that does not match `launch-summary.json`, has a copied fixture that differs from `--dns-fixture`, or contains private-key markers or raw private JWK material in public launch-kit artifacts
