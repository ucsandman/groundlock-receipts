...
- the local `dns-fixture.json` is missing, malformed, for a different domain, for a different demo hash, lacks identity, manifest, chunk, key status, or claim status entries, or has status records that do not match the manifest key/receipt hash
