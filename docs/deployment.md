...
Demo mode does not use live DoH. When `GROUNDLOCK_SIGNER_DOMAIN` is set, it must be a real public DNS host, not a reserved placeholder, local host, malformed name, or IP address. `GROUNDLOCK_DOH_ENDPOINT` is required and must be a valid HTTPS URL so the verifier does not silently fall back to an unintended resolver. `groundlock launch-kit`, `groundlock export-web-env`, `groundlock warm-cache`, and `groundlock check-live` reject malformed, non-HTTPS, credentialed, query-suffixed, fragment-suffixed, non-default-port, reserved placeholder, local, or IP-based launch URLs before making resolver or status requests. They also reject reserved placeholder signer domains from live `--domain` values and DNS fixtures, so a launch bundle cannot emit a `GROUNDLOCK_SIGNER_DOMAIN` that the deployed verifier refuses. The live web verifier and Hacker News readiness audit also reject reserved placeholder launch hosts such as `.example`, `.test`, and `.localhost`. The readiness audit requires the deployed verifier to report `dohEndpointConfigured` and requires `warm-cache`, `check-live`, and `hn_readiness.py` to use the same DoH endpoint URL. The DoH response must include the DNSSEC AD signal; otherwise the verifier fails closed with `dnssec_not_validated`.
...
CI runs `node scripts/smoke_web_response.mjs <base-url> --expected-origin <public-origin> --verify-file-text <demo-pass-text>` against the built Docker image to prove `/`, `/groundlock-receipt-desk.png`, `/robots.txt`, `/sitemap.xml`, `/api/health`, and `POST /api/verify` return through the standalone runtime, production security headers from that contract are present, the homepage uses exact public canonical/share metadata, the share image is served as `image/png` with PNG bytes, discovery files use the public origin and do not expose API/status routes, health and verify responses use `Cache-Control: no-store`, and the demo verifier returns a PASS receipt summary.
CI also starts the same image in live verifier mode with `GROUNDLOCK_SIGNER_DOMAIN`, `NEXT_PUBLIC_SITE_URL`, `GROUNDLOCK_DOH_ENDPOINT`, `GROUNDLOCK_STATUS_BASE_URL`, explicit fetch/rate-limit defaults, and bundled `GROUNDLOCK_STATUS_RECORDS_JSON`. That live smoke requires `/api/health` to report live mode, public canonical/share metadata, the share image route, and discovery files to use the configured HTTPS origin, and the bundled key/claim status endpoints to return active no-store records whose subjects match the requested lookups.
...
- `launch-summary.json` - the launch domain, site URL, content hash, receipt hash, signer key id, DNS TTL, fetch timeout, rate-limit defaults, artifact names, and SHA-256 hashes for the public handoff artifacts.
...
`launch-kit` reconstructs the receipt from the fixture chunks, checks the payload hash, receipt hash, signer domain, key id, active key/claim status records, and refuses to generate the bundle unless the receipt verdict is PASS. Public fixture consumers (`launch-kit`, `export-web-env`, and `warm-cache`) also reject fixtures unless they contain one cache manifest whose DNS name, signer key, receipt hash, declared chunks, reconstructed payload hash, and bundled active key/claim status records all match. They reject fixtures with missing or malformed chunks, malformed status records, ambiguous manifests, private-key markers, or raw private JWK material before copying, querying, or printing launch artifacts. `launch-kit` does not mutate DNS, deploy the verifier, or store private signing keys.
...
In demo mode it returns `200`. In live mode it returns `503` when `GROUNDLOCK_SIGNER_DOMAIN` is set without the required `NEXT_PUBLIC_SITE_URL`, `GROUNDLOCK_STATUS_BASE_URL`, or `GROUNDLOCK_DOH_ENDPOINT`, when the signer domain is malformed, reserved, local, or IP-based, or when the configured site, DoH, or status URLs are malformed, reserved, local, IP-based, or not HTTPS.
...
`--dns-fixture` must be the fixture generated for the same launch domain and the same `--file-or-hash` demo input. The audit checks the fixture JSON before external requests and requires matching `domain`, a valid unambiguous identity TXT whose `kid` matches the manifest key, a valid unambiguous manifest TXT for the demo hash whose signer domain matches the launch domain, every valid unambiguous chunk TXT declared by the manifest `n=<count>`, reconstructed chunk payload that matches manifest `ph`, cached receipt JSON whose signer, top-level `candidateHash`, candidate content hash entry, and body hash match the manifest and demo hash, and valid active `groundlock-status/v1` key/claim status entries whose subjects match the manifest key and receipt hash.

`--launch-kit` is optional for ad hoc checks and recommended for launch. When present, the audit verifies `launch-summary.json`, checks its receipt hash, signer key, receipt verdict, issue time, content class, TXT count, status count, DNS TTL, fetch timeout, and rate-limit defaults against the DNS fixture, checks `dns-zone.txt` TXT values against the DNS fixture and its TTLs against `launch-summary.json`, checks `web.env` against the readiness inputs, validates its fetch timeout and rate-limit values against `launch-summary.json`, checks bundled status records against the DNS fixture, checks `status-records.json` against the DNS fixture key/claim status records, checks that `hn-readiness.ps1` and `runbook.md` call the final audit and live verification commands with the same launch inputs, recomputes the public artifact hashes, compares `checksums.txt`, checks that the copied `dns-fixture.json` matches `--dns-fixture`, scans every root file in the launch-kit directory for private-key markers or raw private JWK material, rejects unexpected root files except an existing `hn-readiness-evidence.json`, and rejects subdirectories before making network calls.

`--evidence-out` is optional but recommended for launch. It writes a versioned JSON report containing the public launch inputs, optional launch-kit path, deployed homepage, share image, verify, robots, and sitemap endpoints, derived key/claim status lookup URLs and canonical record hashes, current git head, security-header contract hash, structured GitHub Actions CI run metadata, and every PASS/FAIL check result. If the output path is inside `--launch-kit`, it must be the root `hn-readiness-evidence.json` file so the audit does not create an unexpected launch-kit artifact after preflight. If the evidence file cannot be written, the audit exits non-zero.

After local preflight and deployed verifier checks, the audit directly fetches `<status-base-url>/key?lookup=key:<signer-domain>:<kid>` and `<status-base-url>/claim?lookup=claim:<receipt-hash>`. Both responses must be HTTP `200`, include `Cache-Control: no-store`, parse as JSON objects, and exactly match the DNS fixture key and claim status records.

The audit exits non-zero if:

- the git worktree is dirty
- `docs/show-hn-draft.md` still contains `LOCAL_DEMO_ONLY`
- the launch URLs are not public HTTPS URLs, include credentials/query/fragment suffixes or non-default ports, contain malformed DNS labels, use a nested health URL path, or the signer domain is still a placeholder/local host or IP address
- the local `dns-fixture.json` is missing, malformed, for a different domain, for a different demo hash, lacks identity, manifest, declared chunk, key status, or claim status entries, has malformed or ambiguous identity/manifest/chunk TXT records, has chunk payload that does not match manifest `ph`, has cached receipt JSON that is malformed or whose top-level `candidateHash`, content hash entries, signer, or body hash do not match the manifest/demo hash, has malformed status records, has a manifest signer domain that does not match the launch domain, has an identity `kid` that does not match the manifest key, or has status records that do not match the manifest key/receipt hash
- `--launch-kit` is provided and the launch kit is missing, has mismatched summary fields, has summary receipt metadata, TXT count, status count, or DNS TTL that does not match the DNS fixture and zone file, has missing or invalid summary fetch timeout or rate-limit defaults, has `dns-zone.txt` TXT records that do not match the DNS fixture, has `web.env` values that do not match the readiness inputs, has invalid `web.env` fetch timeout or rate-limit values, has `web.env` fetch timeout or rate-limit values that differ from `launch-summary.json`, has bundled status records that do not match the DNS fixture, has `status-records.json` entries that do not match the DNS fixture status records, has `hn-readiness.ps1` or `runbook.md` arguments that do not match the readiness inputs, has artifact hashes that do not match the files, has a checksum manifest that does not match `launch-summary.json`, has a copied fixture that differs from `--dns-fixture`, contains private-key markers or raw private JWK material in any root launch-kit file, contains unexpected root files other than `hn-readiness-evidence.json`, or contains subdirectories
- `--evidence-out` points to a directory or would create an unexpected file inside `--launch-kit`
- the latest GitHub Actions `CI` run on `main` is not successful for the current git `HEAD`
- the deployed `/api/health` response is missing, not `ok`, still in demo mode, missing signer domain, site URL, DoH endpoint, or status base URL configuration, has invalid signer domain, site URL, fetch timeout, or rate-limit configuration, or is missing bundled status records when the status base URL shares the verifier origin
...
- the deployed Open Graph/Twitter share image is missing, not served as `image/png`, missing production security headers, or does not return PNG bytes
- deployed `robots.txt` or `sitemap.xml` is missing production security headers, `robots.txt` does not point to the public sitemap and disallow API/status routes, or `sitemap.xml` omits the public homepage or threat model URLs, includes stale placeholder/local origins, exposes API/status paths, or is invalid XML
...
- the public key or claim status endpoint is unreachable, returns non-JSON data, omits `Cache-Control: no-store`, or returns JSON that does not match the DNS fixture status records
...
- Launch URLs and signer domain are real public DNS hosts, not `.example`, `localhost`, malformed DNS names, URL credentials, query/fragment variants, non-default ports, or IP addresses.
...
- Public share image URL returns a production-hardened `image/png` response with PNG bytes.
- `robots.txt` points at the public sitemap and disallows `/api/` and `/groundlock/status/`; `sitemap.xml` lists the public homepage and threat model URLs without API/status paths.
...
- `GROUNDLOCK_STATUS_BASE_URL` serves key and claim status JSON matching the launch fixture, with `Cache-Control: no-store`. If it shares the verifier origin, bundled status records include an active key for `GROUNDLOCK_SIGNER_DOMAIN` and an active `sha256:` claim.
...
- Deployed homepage, robots, sitemap, health, and verify responses include production security headers; health and verify responses include `Cache-Control: no-store`.
...
- `scripts/hn_readiness.py --launch-kit <dir> --evidence-out <path>` writes a JSON evidence report whose `ok` field is `true` and whose CI check includes the exact GitHub Actions run URL, id, status, conclusion, and head SHA for the launched commit.
