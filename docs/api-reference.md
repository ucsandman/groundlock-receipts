...
If `GROUNDLOCK_SIGNER_DOMAIN` is configured, `/api/verify` uses live DNS-over-HTTPS lookups and HTTP status endpoints. Malformed, reserved, placeholder-like, local, or IP-based signer domains return `UNVERIFIABLE`. Missing, malformed, reserved, placeholder-like, non-HTTPS, credentialed, query/fragment-suffixed, malformed-DNS-host, or IP-based `GROUNDLOCK_DOH_ENDPOINT` or `GROUNDLOCK_STATUS_BASE_URL` also returns `UNVERIFIABLE` instead of falling back to demo fixtures, an implicit resolver, or a server error. Without `GROUNDLOCK_SIGNER_DOMAIN`, it uses local demo fixtures.
