...
After the public deployment is live, run the fail-closed launch audit before removing that marker. It checks CI, public HTTPS launch targets, launch-kit checksums and secret scans, live verifier health, homepage canonical/share metadata, public key/claim status endpoints with `Cache-Control: no-store`, resolver-cache warming, live receipt verification, and the deployed `/api/verify` endpoint:
